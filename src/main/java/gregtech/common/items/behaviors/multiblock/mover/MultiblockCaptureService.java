...
public final class MultiblockCaptureService {
    ...
    public static MultiblockSnapshot capture(EntityPlayerMP player, BlockPos controllerPos,
        ...
        if (!(tile instanceof IGregTechTileEntity)) {
            throw new MoverException("gregtech.multiblock_mover.error.not_controller");
        }
        ...
        if (requireInactive && (controller.isActive()
            ...
            throw new MoverException("gregtech.multiblock_mover.error.active");
        }
        ...
        for (BlockPos worldPos : positions) {
            ...
            if (blockTile != null) {
                try {
                    ...
                    if (adapter == null) {
                        throw new MoverException("gregtech.multiblock_mover.error.unsupported_tile");
                    }
                    ...
                    GTLog.logger.error("Multiblock mover tile adapter failed at {} for {}",
                            worldPos, blockTile.getClass().getName(), adapterFailure);
                    throw new MoverException("gregtech.multiblock_mover.error.adapter_failed",
                            adapterFailure);
                }
            }
        ...
        if (blocks.isEmpty()) {
            throw new MoverException("gregtech.multiblock_mover.error.empty_pattern");
        }
        if (blocks.size() > ConfigHolder.multiblockMover.maxBlocks) {
            throw new MoverException("gregtech.multiblock_mover.error.too_large");
        }
    ...
    private static Set<BlockPos> collectPatternPositions(WorldServer world,
        ...
        if (positions.isEmpty()) {
            ...
            if (!isFormed(controller)) {
                throw new MoverException("gregtech.multiblock_mover.error.not_formed");
            }
            collectCommittedPositions(controller, positions);
            collectRuntimePositions(world, controller, positions);
            collectAttachedParts(controller, positions);
    ...
    private static void collectRuntimePositions(WorldServer world,
        ...
        try {
            ...
            GTLog.logger.warn("Could not enumerate GTCEu structure runtime at {}; using compatibility fallbacks",
    ...
    private static void ensureFormed(MultiblockControllerBase controller) throws MoverException {
        ...
        if (!isFormed(controller)) {
            StructureRuntime runtime = controller.getStructureRuntime();
            GTLog.logger.warn("Multiblock mover found unformed controller {} at {}; runtime={}, failure={}",
    ...
    private static boolean isFormed(MultiblockControllerBase controller) {
        ...
        if (runtimeFormed) {
            GTLog.logger.debug("Using GTCEu lifecycle state for formed controller at {}",
                    controller.getPos());
        }
    ...
    private static void validateSpan(Set<BlockPos> positions) throws MoverException {
        ...
        int limit = ConfigHolder.multiblockMover.maxAxisLength;
