...
public final class MultiblockRotationService {
    ...
    public static List<MovingBlockSnapshot> rotate(EntityPlayerMP player,
        ...
        for (MovingBlockSnapshot block : snapshot.getBlocks()) {
            ...
            if (!occupied.add(relative)) {
                throw new MoverException("gregtech.multiblock_mover.error.rotation_collision");
            }
            IBlockState state;
            ...
            try {
                if (tileNbt == null) {
                    ...
                    if (adapter == null) {
                        ...
                        throw new MoverException("gregtech.multiblock_mover.error.rotation_unsupported");
                    }
                    if (sourceTile == null) {
                    ...
                    if (!originalNbt.equals(restoredNbt)) {
                        ...
                        throw new MoverException(
                                "gregtech.multiblock_mover.error.rotation_unsafe_state");
                    }
                }
            } catch (MoverException error) {
                throw error;
            } catch (Throwable error) {
                GTLog.logger.error("Multiblock rotation adapter failed at {} for {}",
                        snapshot.getControllerPos().add(block.getRelativePos()),
                        block.getTileAdapterId(), error);
                throw new MoverException("gregtech.multiblock_mover.error.rotation_unsupported", error);
            }
    ...
    private static void logRejectedBlock(MultiblockSnapshot snapshot,
        ...
        GTLog.logger.warn("Multiblock mover rotation rejected block at {}: {}; adapter={}",
    ...
    private static void validateStateRoundTrip(IBlockState original, IBlockState rotated,
        ...
        if (rotated == null || rotated.getBlock() != original.getBlock()) {
            throw new MoverException("gregtech.multiblock_mover.error.rotation_unsafe_state");
        }
