...
public enum Statistics {
    ...
    public String toString() {
        return TextFormatting.GOLD + translateToLocal(key);
    }

    public String tooltip() {
        return I18n.translateToLocal(tooltipKey);
    }
    ...
    public String calculate(MTEBaseModule module, int fuelFactor, ForgeOfGodsData data, Formatters format) {
        switch (this) {
            case HEAT:
                ...
                return format.format(module.getHeat());
            case EFFECTIVE_HEAT:
                GodforgeMath.calculateMaxHeatForModules(module, data, fuelFactor);
                return format.format(module.getHeatForOC());
            case PARALLEL:
                GodforgeMath.calculateMaxParallelForModules(module, data, fuelFactor);
                return format.format(module.getCalculatedMaxParallel());
            case SPEED_BONUS:
                GodforgeMath.calculateSpeedBonusForModules(module, data);
                return TextFormattingUtil.formatNumbers(module.getSpeedBonus());
            case ENERGY_DISCOUNT:
                GodforgeMath.calculateEnergyDiscountForModules(module, data);
                return TextFormattingUtil.formatNumbers(module.getEnergyDiscount());
            case OC_DIVISOR:
                GodforgeMath.setMiscModuleParameters(module, data);
                return TextFormattingUtil.formatNumbers(module.getOverclockTimeFactor());
            case PROCESSING_VOLTAGE:
                GodforgeMath.calculateProcessingVoltageForModules(module, data, fuelFactor);
                return Formatters.EXPONENT.format(module.getProcessingVoltage());
            default:
                return "";
        }
    }
