...
import gregtech.common.items.behaviors.multiblock.mover.*;

import gregtech.api.util.GTLog;
import gregtech.api.items.multiblock.mover.MovableTileAdapterRegistry;
import gregtech.api.items.multiblock.mover.IRotatableTileAdapter;
import gregtech.api.items.multiblock.mover.MultiblockMovePermissionRegistry;
...
public final class MultiblockMoverRecoveryCommand extends CommandBase {
    ...
    public String getUsage(ICommandSender sender) {
        return "commands.gregtech.mover.usage";
    }
    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args)
        ...
        if (args.length == 1 && "adapters".equalsIgnoreCase(args[0])) {
            sender.sendMessage(new TextComponentTranslation("commands.gregtech.mover.adapters",
                    MovableTileAdapterRegistry.view().size()));
            for (net.minecraft.util.ResourceLocation id : MovableTileAdapterRegistry.view().keySet()) {
                sender.sendMessage(new TextComponentTranslation("commands.gregtech.mover.adapter_entry",
        ...
        if (args.length == 1 && "permissions".equalsIgnoreCase(args[0])) {
            sender.sendMessage(new TextComponentTranslation("commands.gregtech.mover.permissions",
            ...
            for (net.minecraft.util.ResourceLocation id : MultiblockMovePermissionRegistry.view().keySet()) {
                sender.sendMessage(new TextComponentTranslation("commands.gregtech.mover.registry_entry", id));
            }
            return;
        }
        ...
        if (args.length == 2 && "performance".equalsIgnoreCase(args[0])
            ...
            GTLog.logger.info("{} cleared {} multiblock mover performance samples",
                    sender.getName(), cleared);
            sender.sendMessage(new TextComponentTranslation(
                    "commands.gregtech.mover.performance_cleared", cleared));
            return;
        }
        ...
        try {
            ...
            throw new CommandException("commands.gregtech.mover.invalid_id");
        }
        if ("info".equalsIgnoreCase(args[0])) {
            ...
            if (!MultiblockRecoveryJournal.discard(server.getWorld(0), transactionId)) {
                throw new CommandException("commands.gregtech.mover.not_found");
            }
            GTLog.logger.warn("{} discarded multiblock mover recovery journal {}",
                    sender.getName(), transactionId);
            sender.sendMessage(new TextComponentTranslation(
                    "commands.gregtech.mover.discarded", transactionId));
        } else {
    ...
    private static void list(MinecraftServer server, ICommandSender sender) throws CommandException {
        try {
            ...
            sender.sendMessage(new TextComponentTranslation(
                    "commands.gregtech.mover.list", entries.size()));
            for (MultiblockRecoveryJournal.RecoveryEntry entry : entries) {
                sender.sendMessage(new TextComponentTranslation("commands.gregtech.mover.entry",
                        entry.transactionId, entry.dimension, entry.sourceController,
                        entry.targetController, entry.positionCount));
            }
        } catch (IOException error) {
            throw new CommandException("commands.gregtech.mover.read_failed");
        }
    }
    ...
    private static void performance(ICommandSender sender) {
        ...
        sender.sendMessage(new TextComponentTranslation(
                "commands.gregtech.mover.performance_summary",
        ...
        sender.sendMessage(new TextComponentTranslation(
                "commands.gregtech.mover.performance_stages",
    ...
    private static void info(MinecraftServer server, ICommandSender sender, UUID transactionId)
        ...
        try {
            ...
            if (entry == null) throw new CommandException("commands.gregtech.mover.not_found");
            sender.sendMessage(new TextComponentTranslation("commands.gregtech.mover.info",
    ...
    private static void restore(MinecraftServer server, ICommandSender sender, UUID transactionId)
        ...
        if (world == null) throw new CommandException("commands.gregtech.mover.wrong_dimension");
        try {
            int restored = MultiblockRecoveryJournal.restore(world, transactionId);
            GTLog.logger.warn("{} invoked disk recovery for multiblock mover transaction {}",
                    sender.getName(), transactionId);
            sender.sendMessage(new TextComponentTranslation(
                    "commands.gregtech.mover.restored", transactionId, restored));
        } catch (IOException error) {
            throw new CommandException("commands.gregtech.mover.restore_failed");
