...
public class HeatConductorRenderer extends PipeRenderer {

    public static final HeatConductorRenderer INSTANCE = new HeatConductorRenderer();
    private final TextureAtlasSprite[] insulationTextures = new TextureAtlasSprite[6];
    private TextureAtlasSprite wireTexture;

    private HeatConductorRenderer() {
        super("gt_heat_conductor", GTUtility.gregtechId("heat_conductor"));
    }
    ...
    public void buildRenderer(PipeRenderContext renderContext, BlockPipe<?, ?, ?> blockPipe, IPipeTile<?, ?> pipeTile,
        ...
        if (material == null || !(pipeType instanceof HeatConductorType)) {
            return;
        }

        int insulationLevel = ((HeatConductorType) pipeType).insulationLevel;
    ...
    public Pair<TextureAtlasSprite, Integer> getParticleTexture(IPipeTile<?, ?> pipeTile) {
        ...
        if (!(pipeType instanceof HeatConductorType)) {
            return Pair.of(RenderUtil.getMissingSprite(), 0xFFFFFF);
        }
