...
import gregtech.common.items.behaviors.multiblock.mover.PreviewBlockData;
...
final class MoverGhostModelRenderer {
    ...
    static void rebuild(List<PreviewBlockData> blocks) {
        release();
        if (!ConfigHolder.multiblockMover.enableBlockModelPreview
                || blocks.isEmpty()
                || blocks.size() > ConfigHolder.multiblockMover.maxModelPreviewBlocks
                || !OpenGlHelper.useVbo()) {
            return;
        }
        ...
        try {
            ...
            GTLog.logger.warn("Unable to build multiblock mover ghost model; using outline fallback", error);
            release();
        } finally {
    ...
    private static BlockInfo createBlockInfo(PreviewBlockData block) {
        ...
        try {
            ...
            GTLog.logger.debug("Unable to create mover preview MTE {}", metaId, error);
