...
public class FissionReactor {
    ...
    public double computeK(boolean addToEffectiveLists, boolean controlRodsInserted) {
        ...
        for (int i = 0; i < fuelRods.size(); i++) {
            for (int j = 0; j < i; j++) {
                ...
                double resolution = ConfigHolder.nuclear.fissionReactorResolution;
        ...
        for (int i = 0; i < ConfigHolder.nuclear.fissionReactorPowerIterations; i++) {
            normalize(vector);
    ...
    public void computeGeometry() {
        ...
        if (fuelRods.size() > 1) {
            ...
            k = 0.00001;
            maxPower = 0.1 * ConfigHolder.nuclear.nuclearPowerMultiplier;
        }
        /*
    ...
    public int getEffectiveControlRodCount() {
        return effectiveControlRods.size();
    }

    public int getModeratorTippedControlRodCount() {
        int count = 0;
        ...
        for (CoolantChannel channel : coolantChannels) {
            ...
            if (tryFluidDrain != null) {
                ...
                double heatRemovedPerLiter = prop.getSpecificHeatCapacity() /
                        ConfigHolder.nuclear.fissionCoolantDivisor *
                ...
                // Balance turbine output without increasing the core's raw heat generation or warm-up rate.
                double idealOutputFluidUsed = idealFluidUsed * ConfigHolder.nuclear.fissionPowerMultiplier;
                double cappedFluidUsed = Math.min(drained, idealOutputFluidUsed);
                ...
                double thermalFluidUsed = actualFlowRate / ConfigHolder.nuclear.fissionPowerMultiplier;
                this.coolantMass += thermalFluidUsed * prop.getMass();
                heatRemoved += thermalFluidUsed * heatRemovedPerLiter;
            }
        }
    ...
    public void regulateControlRods() {
        ...
        if (pressure > maxPressure * 0.8 || temperature > (coolantExitTemperature + maxTemperature) / 2 ||
            ...
            if (kEff > 0.99) {
                adjustFactor = adjustControlRodReactivity(false, 0.004);
            }
        } else if (temperature > coolantExitTemperature * 0.3 + coolantBaseTemperature * 0.7) {
            if (kEff > 1.01) {
                adjustFactor = adjustControlRodReactivity(false, 0.008);
            } else if (kEff < 1.005) {
                adjustFactor = adjustControlRodReactivity(true, 0.001);
            }
        } else if (temperature > coolantExitTemperature * 0.1 + coolantBaseTemperature * 0.9) {
            if (kEff > 1.025) {
                adjustFactor = adjustControlRodReactivity(false, 0.012);
            } else if (kEff < 1.015) {
                adjustFactor = adjustControlRodReactivity(true, 0.004);
            }
        } else {
            if (kEff > 1.1) {
                adjustFactor = adjustControlRodReactivity(false, 0.02);
            } else if (kEff < 1.05) {
                adjustFactor = adjustControlRodReactivity(true, 0.006);
            }
        }
    ...
    /**
     * Chooses the insertion direction from the actual reactivity curve. Normal rods absorb more as they are
     * inserted, whereas moderator-tipped rods initially increase reactivity when inserted.
     */
    private boolean adjustControlRodReactivity(boolean increaseReactivity, double step) {
        double current = Math.max(0.0, Math.min(1.0, controlRodInsertion));
        double lower = Math.max(0.0, current - step);
        double higher = Math.min(1.0, current + step);
        double currentFactor = ControlRod.controlRodFactor(effectiveControlRods, current);
        double lowerFactor = ControlRod.controlRodFactor(effectiveControlRods, lower);
        double higherFactor = ControlRod.controlRodFactor(effectiveControlRods, higher);

        double target = current;
        if (increaseReactivity) {
            if (lowerFactor < currentFactor && lowerFactor <= higherFactor) {
                target = lower;
            } else if (higherFactor < currentFactor) {
                target = higher;
            }
        } else {
            if (lowerFactor > currentFactor && lowerFactor >= higherFactor) {
        ...
        this.controlRodInsertion = target;
        return true;
    }
