...
public class VirtualContainerRegistry extends WorldSavedData {
    ...
    /**
     * @return the internal Map of containers. Do not use to modify the map!
     */
    ...
    /**
     * Retrieves a container from the registry, creating it with {@link #DEFAULT_SIZE the default size} if it does not
     * exist
     *
     * @param key  The name of the container
     * @param uuid The uuid of the player the container is private to, or null if the container is public
    ...
    public static void addContainer(String key, UUID uuid, int size) {
        if (containerMap.containsKey(uuid) && containerMap.get(uuid).containsKey(key)) {
            GTLog.logger.warn("Overwriting virtual container " + key + "/" + (uuid == null ? "null" : uuid.toString()) +
                    ", this might cause item loss!");
    ...
    public static void delContainer(String key, UUID uuid, boolean removeContainer) {
        if (containerMap.containsKey(uuid) && containerMap.get(uuid).containsKey(key)) {
            ...
            GTLog.logger.warn("Attempted to delete container " + key + "/" + (uuid == null ? "null" : uuid.toString()) +
    ...
    public static void initializeStorage(World world) {
        ...
        VirtualContainerRegistry instance = (VirtualContainerRegistry) storage.getOrLoadData(
                VirtualContainerRegistry.class, DATA_ID);
    ...
    protected static class VirtualContainer
