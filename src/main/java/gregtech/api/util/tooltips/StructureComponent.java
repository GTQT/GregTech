...
public class StructureComponent extends AbstractTooltipComponent {
    ...
    public void addInformation(MetaTileEntity metaTileEntity, List<String> tooltip) {
        ...
        if (metaTileEntity instanceof MultiblockControllerBase mte && mte.structurePattern != null && TooltipHelper.isShiftDown()) {
            ...
            tooltip.add(I18n.format("gregtech.multiblock.structure_size.tooltip",
                    mte.structurePattern.getStructureXSize(),
                    mte.structurePattern.getStructureYSize(),
                    mte.structurePattern.getStructureZSize()));
            if(tier > 1)
