## Welcome to GregTech CE: MeowmelMuku
### What is this?
This is a fork version of GregTech CE Unofficial (GTCEu) based on version 2.9, dedicated to enriching more content and ensuring that addon development is not affected by frequent underlying changes in the original mod.

### About This Fork
This fork builds upon the solid foundation of GTCEu 2.9 while introducing significant enhancements and optimizations:

### Project Goal
To provide a stable and feature-rich GregTech experience based on GTCEu 2.9, focusing on content expansion while maintaining compatibility with existing addons and preventing disruptions from core mod changes.

### Original License:
Source: **[GregTechCEu](https://github.com/GregTechCEu/GregTech)**.

License: **[GregTechCEu.LICENSE](https://github.com/GregTechCEu/GregTech/blob/master/LICENSE)**.


## Credited Works
Heating Coil Textures, Wooden Forms, World Accelerators, and the Extreme Combustion Engine are from the **[GregTech: New Horizons Modpack](https://www.curseforge.com/minecraft/modpacks/gt-new-horizons)**.

Primitive Water Pump and Super Tank GUI Textures are from the **[IMPACT: GREGTECH EDITION Modpack](https://gt-impact.github.io/)**.

Ender Fluid Link Cover, Auto-Maintenance Hatch, Optical Fiber, and Data Bank Textures are from **[TecTech](https://github.com/Technus/TecTech)**.

Steam Grinder is from **[GregTech++](https://www.curseforge.com/minecraft/mc-mods/gregtech-gt-gtplusplus)**.

Certificate of Not Being a Noob Anymore is from **[Crops++](https://www.curseforge.com/minecraft/mc-mods/berries)**.

NanoMuscle and QuarkTech Suite Armors are from **[Nukepowered Utils](https://www.curseforge.com/minecraft/mc-mods/nukepowered-utils)**.

OpenComputers compatibility has some code from **[GTCE2OC](https://www.curseforge.com/minecraft/mc-mods/gtce2oc)**.

Some code is from **[SussyPatches](https://github.com/MCTian-mi/SussyPatches)**.

Some material and casing textures are from **[zedtech-ceu](https://github.com/brachy84/zedtech-ceu)** (CC BY-NC-SA 4.0), originally by Zerrens for Zederrian Technology GT5U, with additional textures and models by Puzzlebark.

Overclock Hatch and Accelerate Hatch textures are from **[GregTech-Odyssey](https://github.com/GregTech-Odyssey/GregTech-Odyssey)** (CC BY-NC-SA 4.0).

See something we forgot to credit? Reach out to us on Discord, or open an issue and ask for appropriate credit, we will happily mark it here
