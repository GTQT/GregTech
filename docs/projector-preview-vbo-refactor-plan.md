...
| 瓶颈点 | 原因 |
|--------|------|
| `BlockRendererDispatcher.renderBlock()` | 每个方块走完整 MC 渲染管线（模型查找→纹理绑定→顶点生成） |
| 4 层 `BlockRenderLayer` 各遍历一次 | 表面方块 × 4 次完整渲染 |
| `glNewList(GL_COMPILE)` | Display List 是 OpenGL 3.0 已废弃的 API |
| 无增量更新 | 每次重建全量编译所有方块几何 |
...
| 状态变量（`mbpPos`, `mbpEndTime`, `layer`, `compareMode`, `channelValues`） | `MultiblockPreviewRenderer` | `PreviewState` 共享状态类 |
...
**复用已有代码**：
- `FaceCulledRenderBlocks` — 已存在于 `godforge/util/`，可直接复用
...
GhostBlockRenderer.renderGhostPreview(controller, durTimeMillis)
  ...
  └─ 4. 每帧渲染（renderWorldLastEvent）
        bindTexture(white1x1)  // 纯白纹理
        setupBlending(0.5 alpha)
...
1. `MultiblockControllerBase.onRightClick()`
   ```java
   // 改造前
...
2. `StructureProjectorBehavior.onItemUseFirst()`
   ```java
   // 改造前
...
| `renderControllerInList()` | 重命名为 `buildControllerVBO()` |
| `renderPieceInList()` | 重命名为 `buildPieceVBO()` |
| 旧 `renderMultiBlockPreview()` | 保留为 deprecated，内部转发到新方法 |
...
| `PreviewState.java` | **新建** | 共享渲染状态（位置、时间、layer、模式标志） |
